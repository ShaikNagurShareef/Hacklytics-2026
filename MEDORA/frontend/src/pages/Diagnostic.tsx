...
import { postDiagnosticChat } from "@/api/client";

const WELCOME_TAGLINE = "I can help analyse medical images and generate reports. Attach an image or describe the study.";
const QUICK_REPLIES = [
  "Analyse this image",
  "Generate a report",
...
export function Diagnostic() {
  ...
  const handleSend = useCallback(
    async (query: string, imageBase64?: string, imageMimeType?: string) => {
      ...
      setError(null);
      try {
        const res = await postDiagnosticChat({
        ...
        const pdfData = res.metadata?.report_pdf as string | undefined;
        addAssistantMessage(res.content, res.agent_name, pdfData);
      } catch (e) {
    ...
    >
      <div className="flex flex-col flex-1 min-h-0">
        <ChatView
          ...
          placeholder="Describe the study or attach an image…"
          allowImage
          welcomeTagline={WELCOME_TAGLINE}
          welcomeIcon={ScanLine}
        />
        <Footer />
      </div>
    </ChatLayout>
