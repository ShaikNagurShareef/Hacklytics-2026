...
Report structure follows the ACR Practice Parameter for Communication of
Diagnostic Imaging Findings. Section order and names are defined in
acr_report_structure.py (single source of truth).
"""
...
# =====================================================================
# Report formatting prompt — ACR Practice Parameter structure
# =====================================================================

def _build_acr_report_prompt() -> str:
    """Build report generation prompt from ACR section definitions (acr_report_structure.py)."""
    from app.agents.diagnostic.acr_report_structure import (
        ACR_CRITICAL_FINDINGS_DISCLAIMER,
        ACR_SECTIONS,
        AI_REPORT_DISCLAIMER,
    )
    section_instructions = {
        "clinical_indication": "Reason for study; or 'Not provided'.",
        "comparison": "Prior studies used; or 'None'.",
        "technique": "Views/sequences/contrast; note if quality limits interpretation.",
        "findings": "Systematic by anatomy; normal briefly, abnormal in detail; measurements when estimable.",
        "impression": "Numbered key conclusions; BI-RADS/Lung-RADS/etc. where applicable.",
        "recommendations": "Follow-up imaging or 'Clinical correlation. No additional imaging at this time.'",
        "critical_findings": "If none: 'None.' If any: list and add immediate-communication statement per ACR.",
    }
    lines = [
        "You are generating a one-page diagnostic imaging report that follows the "
        "ACR Practice Parameter for Communication of Diagnostic Imaging Findings. "
        "Output ONLY the sections below, in this exact order. No decorative lines, "
        "no tables, no filler. Standard medical terminology only.",
        "",
        "Structure:",
        "",
        "## DIAGNOSTIC IMAGING REPORT",
        "**Report ID:** [use Report ID provided below]",
        "**Examination Date:** [YYYY-MM-DD]",
        "**Examination Type:** [Modality and body region, e.g. CT CHEST, MRI BRAIN]",
        "**Patient:** [Age, Gender if provided; else 'Not provided']",
        "",
    ]
    for key, title, _desc in ACR_SECTIONS:
        if title == "DEMOGRAPHICS":
            continue
        instr = section_instructions.get(key, "Content as appropriate.")
    ...
    lines.extend([
        ...
        "",
        f"*{AI_REPORT_DISCLAIMER}*",
        "",
        "Rules:",
        f"- CRITICAL FINDINGS: if any, add: '{ACR_CRITICAL_FINDINGS_DISCLAIMER}'",
        "- One page only; no extra headers or blank lines.",
    ])
    return "\n".join(lines)


REPORT_GENERATION_PROMPT = _build_acr_report_prompt()
